/*
	Rollup.js v0.26.7
	Sun Jun 05 2016 17:10:05 GMT-0400 (EDT) - commit 7f1d9f26914f80afd5395461b18f1d1bd979f4b1
...
var Module = function Module (ref) {
	...
	this.comments = [];
	this.statements = this.parse( ref.ast );
...
Module.prototype.bindImportSpecifiers = function bindImportSpecifiers () {
	var this$1 = this;
		...
		keys( specifiers ).forEach( function ( name ) {
			...
			specifier.module = this$1.bundle.moduleById[ id ];
		});
	});

	this.exportAllModules = this.exportAllSources.map( function ( source ) {
		var id = this$1.resolvedIds[ source ];
		return this$1.bundle.moduleById[ id ];
	});

	this.sources.forEach( function ( source ) {
		var id = this$1.resolvedIds[ source ];
		var module = this$1.bundle.moduleById[ id ];
...
Module.prototype.parse = function parse$1 ( ast ) {
...
function resolveId ( importee, importer ) {
	...
	if ( importer === undefined ) return addJsExtensionIfNecessary( resolve$1( process.cwd(), importee ) );

	// external modules are skipped at this stage
	if ( importee[0] !== '.' ) return null;

	return addJsExtensionIfNecessary( resolve$1( dirname( importer ), importee ) );
}
...
function transform ( source, id, transformers ) {
	...
	.then( function ( code ) { return ({ code: code, originalCode: originalCode, ast: ast, sourceMapChain: sourceMapChain }); } )
	.catch( function ( err ) {
		err.id = id;
...
var Source = function Source ( index ) {
...
function collapseSourcemaps ( map, modules, bundleSourcemapChain ) {
	...
	var ref = source.traceMappings(), names = ref.names, mappings = ref.mappings;

	// we re-use the `map` object because it has convenient toString/toURL methods
	map.sourcesContent = modules.map( function ( module ) { return module.originalCode; } );
...
Bundle.prototype.fetchModule = function fetchModule ( id, importer ) {
	...
	return this.load( id )
		...
		.then( function ( source ) { return transform( source, id, this$1.transformers ); } )
		.then( function ( source ) {
			var code = source.code, originalCode = source.originalCode, ast = source.ast, sourceMapChain = source.sourceMapChain;

			var module = new Module({ id: id, code: code, originalCode: originalCode, ast: ast, sourceMapChain: sourceMapChain, bundle: this$1 });

			this$1.modules.push( module );
			this$1.moduleById[ id ] = module;
		...
		return this$1.resolveId( source, module.id )
			.then( function ( resolvedId ) {
				...
				if ( resolvedId ) {
					...
					externalName = resolve$1( module.id, '..', source );
				}
...
Bundle.prototype.render = function render ( options ) {
	...
	if ( options.sourceMap ) {
		...
		if ( file ) file = resolve$1( typeof process !== 'undefined' ? process.cwd() : '', file );
